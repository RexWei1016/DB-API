...
exercise_record_model = exercise_record_api.model('ExerciseRecord', {
    ...
    'exe_time': fields.String(required=True, description='Exercise time (HH:MM:SS)'),
...
@exercise_record_api.route('/<string:user_id>/<string:exe_date>/<string:exe_time>')
class DeleteExerciseRecord(Resource):
    def delete(self, user_id, exe_date, exe_time):
        """
        刪除指定的運動紀錄
        """
        response, status = ExerciseRecordController.delete_record(user_id, exe_date, exe_time)
        return response, status
